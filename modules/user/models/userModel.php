<?php
function get_user_id($field, $value)
{
    $query_string = "SELECT user_id FROM `tbl_users` WHERE `{$field}` = '{$value}'";
    $row = db_fetch_row($query_string);
    return $row['user_id'];
}
...
function get_num_active_user()
{
    return db_num_rows("SELECT * FROM `tbl_users` WHERE `status` = 'active'");
}

function get_num_deleted_user()
{
    return db_num_rows("SELECT * FROM `tbl_users` WHERE `status` = 'deleted'");
}
...
function get_reset_time($reset_token)
{
    $query_string = "SELECT reset_time FROM `tbl_users` WHERE `reset_token` = '{$reset_token}'";
    $row = db_fetch_row($query_string);
